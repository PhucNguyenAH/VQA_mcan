...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        log_path = f"./{self.__C.MODEL}_train_log.csv"
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            logfile.write(
                'epoch = ' + str(epoch_finish) +
                '  loss = ' + str(loss_sum / (data_size * self.__C.GRAD_ACCU_STEPS)) +
                '\n' +
                'lr = ' + str(optim._rate) +
                '\n\n'
            )
            logfile.close()
            logs = {}
            logs["epoch"] = epoch_finish
            logs["loss"] = loss_sum / (data_size * self.__C.GRAD_ACCU_STEPS)
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        for step, (
            ...
            print("self.__C.EVAL_BATCH_SIZE",self.__C.EVAL_BATCH_SIZE)
            print(ques_ix_iter.size())
            print(ques_ix_iter)
            img_feat_iter = img_feat_iter.cuda()
            ques_ix_iter = ques_ix_iter.cuda()
            print(img_feat_iter.size())
            pred = net(
                img_feat_iter,
                ques_ix_iter
            )
        ...
        if valid:
            ...
            plt.xticks(range(len(vqaEval.accuracy['perQuestionType'])), vqaEval.accuracy['perQuestionType'].keys(), rotation='60',fontsize=10)
